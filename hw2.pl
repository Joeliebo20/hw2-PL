...
%:- Based on syllabus must say if you have collaborated with someone 
%:- Discussed bubblesort algorithm with Bradley Estus
...
%:- 1.)
%:-   Clauses:
%:-   Predicates: vertical(), horizontal, line(), point()
%:-   Rules:
%:-   Facts:
%:- 2.)
%:-   Atoms: _
%:-   Variables: X, Y, Z
%:-   Data Structure Constructors: point(X, Y), line(X, Y) where X and Y are points
/* Problem 1b:
...
%:- 1.) 
%:-   Clauses: numeral(0)., numeral(succ(X)), Y, succ(Z)) :- numeral(X), add(0, Y, Y), add(succ(X),Y,succ(Z)) :- add(X,Y,Z).
%:-   Predicates: numeral(), add()
%:-   Rules: numeral(succ(X)) :-  numeral(X)., add(succ(X),Y,succ(Z)) :- add(X,Y,Z).
%:-   Facts: numeral(0)., add(0, Y, Y).
%:- 2.)
%:-   Atoms: 0
%:-   Variables: X, Y, Z
%:-   Data Structure Constructors: numeral()
/* Problem 2:
...
%:- my_first(3, [4, 3, 2, 1]). % FAIL
%:- my_last(a, [a]). % SUCCEED
%:- my_last(1, [3, 2, 1]). % SUCCEED
%:- my_last(1, [3, 2, 1, 0]). % FAIL
...
element_at(X, [Y|_], 0) :- X == Y.
element_at(X, [_|Ys], N) :- N1 is N - 1, element_at(X, Ys, N1).
...
insert_at(X, [Y|Ys], 0, [X|Zs]).
insert_at(X, [_|Ys], N, [_|Zs]) :- N1 is N - 1, insert_at(X, Ys, N1, Zs).
...
delete_at([], _, _, []).
delete_at(E, [_|Xs], N, [_|Zs]) :- N1 is N + 1 , delete_at(E,Xs,N1,Zs).
delete_at(E, [X|Xs], N, [X|Zs] ) :- N1 is N + 1 , delete_at(E,Xs,N1,Zs).
...
zip2([], [_|_], []).
zip2([_|_], [], []).
zip2([X|Xs], [Y|Ys], [Z|Zs]) :- ((X, Y) = Z), !, zip2(Xs, Ys, Zs).
...
has_subseq(_, []) :- true, !.
has_subseq([X|Xs], [Y|Ys]) :- X =\= Y, has_subseq(Xs, [Y|Ys]), !.
...
swap([Y, X|Zs], [X, Y|Zs]) :- X < Y, !.
swap([X|Xs], [X|Xs2]) :- swap(Xs, Xs2), !.

bubble(X, Y) :- swap(X, Z), bubble(Z, Y), !.
bubble(X, X) :- true, !.

bubblesort([], []).
bubblesort(X, Z) :- bubble(X, Y), Z == Y.
...
%:- bubblesort([4, 3, 2, 1],[1, 2, 4, 3]).     % FAIL
...
merge([], [], []).
merge(X, Y, Z) :- append(X, Y, Temp), bubblesort(Temp, Z).
